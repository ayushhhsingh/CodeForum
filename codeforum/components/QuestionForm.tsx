...
import { getAppwriteErrorMessage } from "@/lib/appwrite-error";
...
const QuestionForm = ({ question }: { question?: QuestionFormDocument }) => {
    ...
    const [formData, setFormData] = React.useState({
        ...
        authorId: String(question?.authorId || user?.$id || ""),
    ...
    const attachmentMessage =
        ...
        (question?.attachmentId
            ...
            : "PNG, JPG, GIF, and WEBP are supported up to 10MB.");
    ...
    const validateForm = () => {
        ...
        if (!formData.content.trim()) {
            ...
            missingFields.push("details must be at least 20 characters");
        }

        if (!formData.authorId) {
            missingFields.push("login session");
        }

        if (missingFields.length > 0) {
            return `Please fill these fields: ${missingFields.join(", ")}`;
        }
    ...
    const submit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setError(() => getAppwriteErrorMessage(error));
        }
    ...
    return (
        <form className="space-y-4" onSubmit={submit}>
            ...
            <LabelInputContainer>
                ...
                />
                <small className="text-white/60">{attachmentMessage}</small>
            </LabelInputContainer>
            <LabelInputContainer>
                <Label htmlFor="tag">
                    Tags
